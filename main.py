    ...
    html_content = """
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .product-card { background: white; border-radius: 15px; padding: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); transition: transform 0.3s; position: relative; }
                ...
                .product-actions { margin-top: 15px; display: flex; gap: 10px; flex-wrap: wrap; }
                ...
                .modal-content { background-color: white; margin: 10% auto; padding: 30px; border-radius: 15px; width: 90%; max-width: 500px; }
                .modal-content h3 { margin-bottom: 20px; }
                .modal-content input, .modal-content textarea { width: 100%; padding: 10px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 8px; }
        ...
        <body>
            <div class="container">
                ...
                <div class="tabs">
                    <button class="tab active" onclick="showTab('products', event)">📦 Все товары</button>
                    <button class="tab" onclick="showTab('myProducts', event)">🏪 Мои товары</button>
                ...
                <div id="productsTab" class="tab-content active">
                    <div class="cart-section">
                        <div class="section-header">
                            <h2>📦 Все товары</h2>
                            <button class="add-product-btn" onclick="showAddProductModal()">➕ Добавить свой товар</button>
                        </div>
                ...
                <div id="myProductsTab" class="tab-content">
                    <div class="cart-section">
                        <div class="section-header">
                            <h2>🏪 Мои товары</h2>
                            <button class="add-product-btn" onclick="showAddProductModal()">➕ Добавить товар</button>
                        </div>
            ...
            <div id="productModal" class="modal">
                <div class="modal-content">
                    ...
                    <input type="number" id="productStock" placeholder="Количество на складе" value="1">
                    <button class="btn btn-primary" onclick="saveProduct()" style="width: 100%;">Сохранить</button>
                </div>
            </div>
            ...
            <script>
                ...
                async function login() {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            currentUser = { username: data.username, id: data.user_id, is_admin: data.is_admin };
                            document.getElementById('authPanel').style.display = 'none';
                            document.getElementById('userPanel').style.display = 'block';
                ...
                async function loadProducts() {
                    ...
                    try {
                        ...
                        if (products.length === 0) { 
                            grid.innerHTML = '<div style="text-align:center; color:white;">Нет товаров</div>'; 
                            return; 
                        }
                        grid.innerHTML = products.map(p => `
                            <div class="product-card">
                                ...
                                <div class="stock">Продавец: ${escapeHtml(p.owner_username || 'неизвестен')}</div>
                ...
                async function loadMyProducts() {
                    ...
                    try {
                        ...
                        products = products.filter(p => p.user_id === currentUser.id);
                        const grid = document.getElementById('myProductsGrid');
                        if (products.length === 0) { 
                            grid.innerHTML = '<div style="text-align:center; padding:40px; color:#999;">У вас пока нет товаров. Нажмите "Добавить товар"</div>'; 
                            return; 
                        }
                        grid.innerHTML = products.map(p => `
                            <div class="product-card">
                                ...
                                <div class="product-actions">
                                    <button class="btn btn-warning" onclick="editProduct(${p.id})">✏️ Редактировать</button>
                ...
                async function editProduct(productId) {
                    try {
                        const response = await fetch(API_URL + '/products/' + productId, { headers: { 'Authorization': 'Bearer ' + token } });
                        ...
                        container.innerHTML = cart.items.map(item => `
                            <div class="cart-item">
                                <div>
                                    ...
                                    <div>${Number(item.product_price).toLocaleString()} ₽ × ${item.quantity}</div>
                                </div>
                                <div>
                                    <strong>${Number(item.product_price * item.quantity).toLocaleString()} ₽</strong>
                                    <button class="btn btn-danger" style="margin-left: 10px;" onclick="removeFromCart(${item.id})">Удалить</button>
                                </div>
                            </div>
                        `).join('');
                ...
                async function removeFromCart(itemId) {
                    try { 
                        await fetch(API_URL + '/cart/remove/' + itemId, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } }); 
                        loadCart(); 
                    } catch(e) { 
                        alert('Ошибка'); 
                    }
                    ...
                    try {
                        ...
                        container.innerHTML = orders.map(order => `
                            <div class="order-card">
                                ...
                                <div>Сумма: ${Number(order.total_price).toLocaleString()} ₽</div>
                                <div>Товаров: ${order.items_count}</div>
                                <div>Дата: ${new Date(order.created_at).toLocaleString()}</div>
                                ${order.status === 'created' ? '<button class="btn btn-primary" style="margin-top:10px;" onclick="payOrder(' + order.id + ')">Оплатить</button>' : ''}
                            </div>
                        `).join('');
                    } catch(e) { console.error(e); }
                }
                ...
                async function payOrder(orderId) {
                    try {
                        const response = await fetch(API_URL + '/orders/id/' + orderId + '/pay', { 
                            method: 'POST', 
                            headers: { 'Authorization': 'Bearer ' + token } 
                        });
                        if (response.ok) { 
                ...
                function showTab(tabName, event) {
                    ...
                    if (tabName === 'myProducts' && token && currentUser) loadMyProducts();
                }
